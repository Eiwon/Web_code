...
public class PathFilter extends HttpFilter implements Filter{
	...
	private static String[] validList = {
			...
			"/memberResult.jsp", "/memberUpdate.jsp"
			};
	...
	protected void doFilter(HttpServletRequest req, HttpServletResponse res, FilterChain chain)
		...
		if(!validPath.contains(reqPath)) { // 존재하지 않는 경로로 접근시
			...
			isError = true;
		}else if(sessionPath.contains(reqPath) && !req.isRequestedSessionIdValid()) { // 세션 만료시
			System.out.println("세션 만료");
			msg = "세션이 만료되었습니다.";
			isError = true;
		}else if(sessionPath.contains(reqPath) && req.getSession().getAttribute("userId") == null) {
